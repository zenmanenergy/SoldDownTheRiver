...
def get_transactions_by_location_id(location_id):
	...
	query = f"""
		...
		LEFT JOIN (
			SELECT th1.TransactionId, GROUP_CONCAT(CONCAT(h1.FirstName, ' ', h1.LastName) SEPARATOR ', ') AS Notary
		...
		LEFT JOIN (
			SELECT th2.TransactionId, GROUP_CONCAT(CONCAT(h2.FirstName, ' ', h2.LastName) SEPARATOR ', ') AS Sellers
		...
		LEFT JOIN (
			SELECT th3.TransactionId, GROUP_CONCAT(CONCAT(h3.FirstName, ' ', h3.LastName) SEPARATOR ', ') AS Buyers
