...
def import_shippingagents():
	# Connect to the database
	cursor, connection = ConnectToDatabase()

	# Execute the query to fetch ShippingAgent
	query = "SELECT DISTINCT ShippingAgent FROM raw_manifest WHERE ShippingAgent <> '' AND ShippingAgent IS NOT NULL;"
	...
	processed_result = []
	for row in result:
		shippingAgent_name = row['ShippingAgent']
		parts = shippingAgent_name.split(',', 1)  # Split into LastName and the rest
		...
		# Update the raw_manifest table to set ShippingAgent_humanId
		update_query = """
			UPDATE raw_manifest 
			SET ShippingAgent_humanId = %s 
			WHERE ShippingAgent = %s;
		"""
		cursor.execute(update_query, (human_id, shippingAgent_name))
		connection.commit()

		# Add the result with HumanId
		processed_result.append({
			"ShippingAgent": shippingAgent_name,
