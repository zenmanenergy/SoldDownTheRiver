...
def import_owners():
	...
	# Execute the query to fetch OwnersName
	query = "SELECT DISTINCT OwnersName FROM raw_manifest WHERE OwnersName <> '' AND OwnersName IS NOT NULL;"
	...
	processed_result = []
	for row in result:
		owners_name = row['OwnersName']
		parts = owners_name.split(',', 1)  # Split into LastName and the rest
		...
		update_query = """
			...
			WHERE OwnersName = %s;
		"""
		cursor.execute(update_query, (human_id, owners_name))
		connection.commit()
