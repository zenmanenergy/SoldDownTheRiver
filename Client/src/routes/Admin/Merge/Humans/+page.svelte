...
<script>
	...
	function openHumanInNewTab(humanId) {
		if (selectedHumans.length === 2) {
			...
			const win1 = window.open(`/Admin/Human?HumanId=${human1.HumanId}&mergeHumanId=${human2.HumanId}`, '_blank1');
			setTimeout(() => {
				const win2 = window.open(`/Admin/Human?HumanId=${human2.HumanId}&mergeHumanId=${human1.HumanId}`, '_blank2');
			}, 500);
		} else {
