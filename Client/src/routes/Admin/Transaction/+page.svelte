		...
		<div class="ActionBox">
			...
			{#if transactionId}
				...
				<div class="add-human-button">
					<button class="button is-primary" on:click={() => window.location.href = `/Admin/Human?HumanId=&TransactionId=${transactionId}`}>
						Add a New Human
					</button>
				</div>
					...
					<table>
						...
						<tbody>
							{#each displayedAvailableHumans as human}
								<tr>
									<td on:click={() => window.open(`/Admin/Human?HumanId=${human.HumanId}`, '_blank')}>
